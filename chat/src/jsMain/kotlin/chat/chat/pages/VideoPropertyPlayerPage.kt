...
@Composable
fun VideoPropertyPlayerPage(
    id: String ,
    modifier: Modifier = Modifier,
) {
    val pageId : String = remember { mutableStateOf(id).toString() }
    VideoPropertyPlayerPageImpl(id, modifier)
}

@Composable
fun VideoPropertyPlayerPageImpl(
    id: String ,
    modifier: Modifier = Modifier,
) {
    console.info( "PropertyPlayerPage - Id = "+id)
    val navigator = LocalNavigator.current
    val properties = convertPropertyJsonStringToList ( propertyJsonString )
    val propertyList = properties.filter ({it.id == id.toInt() } )
