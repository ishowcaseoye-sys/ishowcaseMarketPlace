...
import chat.chat.components.widgets.ShortThumbnailCard
import chat.chat.components.widgets.ShortThumbnailCardDefaults
import chat.chat.components.widgets.SubscribeButton
import chat.chat.data.ShortsDataProvider
...
@Composable
fun ShortsPropertyGrid(showPersonalisedFeedDialogState: MutableState<Boolean>) {
...
@Composable
fun ShortPropertyDetails(id: String, onBackPressed: () -> Unit) {
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        Div(
            ...
            allShorts.forEach { details ->
                ShortPropertyVideoPlayer(details = details, rootElement = containerRef)
            }
...
@Composable
private fun ShortPropertyVideoPlayer(
                    ...
                    if (isSmallBreakpoint) {
                        Column(
                            modifier = Modifier.margin(
                                right = ShortPropertyActionsDefaults.WIDTH + 16.px
                            ),
...
@Composable
private fun ShortPropertyActions(
        ...
        ShortPropertyAction.entries.forEach { action ->
            Column(
                ...
                AssetImageButton(
                    ...
                    asset = when (action) {
                        ShortPropertyAction.Like ->
                            if (isLiked) Asset.Icon.LIKED_SELECTED else Asset.Icon.LIKED

                        ShortPropertyAction.Dislike ->
                            if (isDisliked) Asset.Icon.DISLIKE_SELECTED else Asset.Icon.DISLIKE

                        ShortPropertyAction.Comment -> Asset.Icon.COMMENTS
                        ShortPropertyAction.Share -> Asset.Icon.SHARE
                        ShortPropertyAction.More -> Asset.Icon.MORE
                    },
                    onClick = when (action) {
                        ShortPropertyAction.Like -> onLike
                        ShortPropertyAction.Dislike -> onDislike
                        ShortPropertyAction.Comment -> onOpenComments
                        ShortPropertyAction.Share -> onShare
                        ShortPropertyAction.More -> onOpenMore
                    }
                )

                if (action != ShortPropertyAction.More && !isShortWindowState.value) {
                    Box(Modifier.fontWeight(FontWeight.Medium).fontSize(16.px)) {
                        Text(
                            when (action) {
                                ShortPropertyAction.Like -> likeCount
                                ShortPropertyAction.Dislike -> dislikeCount
                                ShortPropertyAction.Comment -> commentCount
                                ShortPropertyAction.Share -> shareCount
                                ShortPropertyAction.More -> 0
                            }.toString(),
                        )
                    }
                }
...
private object ShortPropertyActionsDefaults {
    val WIDTH = 56.px
}

private enum class ShortPropertyAction { Like, Dislike, Comment, Share, More }
