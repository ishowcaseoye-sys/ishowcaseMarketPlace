...
import chat.chat.components.widgets.player.PlayerAndCommentsVehicle
...
@Composable
fun VideoVehiclePlayerPage(
    id: String ,
    modifier: Modifier = Modifier,
) {
    val pageId : String = remember { mutableStateOf(id).toString() }
    VideoVehiclePlayerPageImpl(id, modifier)
}

@Composable
fun VideoVehiclePlayerPageImpl(
    id: String ,
    modifier: Modifier = Modifier,
) {
    val navigator = LocalNavigator.current
    val vehicles = convertVehicleJsonStringToList ( vehicleJsonString )
    val vehicleList = vehicles.filter ( {it.id == id.toInt()})
    var vehicle : Vehicle = vehicleList.get(0)
    val videoId = BASE_URL + FILES + vehicle.video1
    ...
    val segmentContent = remember {
        movableContentOf { modifier: Modifier ->
            SegmentedContent(category = "Vehicles", state = selectedSegment, modifier = modifier)
        }
    }

    Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.TopEnd) {
         Row(modifier = Modifier.fillMaxSize().then(modifier)) {
            PlayerAndCommentsVehicle(
