...
@Composable
fun AllElectronicPage(query: String,) {
    ...
    Column(modifier = Modifier.padding(top=90.px)) {
        ...
        ShortSuggestionsElectronics(
            shorts =  remember(feedProvider) { feedProvider.getAllElectronicsFeed().filter { it.typeId==2 }
            }
        )

        Box(modifier = Modifier.height(30.px))

        MainAllElectronicGrid(
            electronics = remember(feedProvider) { feedProvider.getAllElectronicsFeed()
                .filter { it.typeId==1 }
            }
        )
    }
...
@Composable
fun MainAllElectronicGrid(
    electronics: List<Electronic>,
    modifier: Modifier = Modifier,
    gridGap: GridGap = GridGap(20.px),
    minWidth: CSSLengthOrPercentageNumericValue = 250.px,
) {
    console.info( "AXA - electronics.size = "+electronics.size)
