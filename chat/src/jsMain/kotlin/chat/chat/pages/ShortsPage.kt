...
fun ShortDetails( id: String, onBackPressed: () -> Unit) {
    ...
    console.info("AXA - ShortDetails 1, id = $id")

    val isSmallBreakpoint by rememberIsSmallBreakpoint()
    var containerRef by remember { mutableStateOf<Element?>(null) }
    console.info("AXA - ShortDetails 2")
    // Data States
    val shortsDataProvider = remember { ShortsDataProvider() }
    console.info("AXA - ShortDetails 3")
    val suggestedShorts = remember(shortsDataProvider) {
        shortsDataProvider.getPremiumVehicleFeed()
    }
    console.info("AXA - ShortDetails 4")
    //val short =  suggestedShorts.filter( vehicle.id == id.toInt()  )
    val short = remember(shortsDataProvider) { shortsDataProvider.getOnePremiumVehicle(id.toString()) }
    console.info("AXA - ShortDetails 5")

    console.info("AXA - ShortDetails 6")
    ...
    console.info("AXA - ShortDetails 7")
    Box(modifier = Modifier.fillMaxSize()) {
        Div(
            ...
            allShorts.forEach { details ->
                ShortVideoPlayer( details = details as Vehicle, rootElement = containerRef)
            }
        }
                            ...
                            Row(
                                ...
                                Column(
                                    ...
                                    TextBox(
                                        ...
                                        size = 14,
                                        text = "${details.description} subscribers",
                                    )
                                }
                            }
...
internal object ShortActionsDefaults {
    val WIDTH = 56.px
}
