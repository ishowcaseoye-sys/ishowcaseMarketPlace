...
@Composable
fun AllFashionPage(query: String,) {
    ...
    Column(modifier = Modifier.padding(top=90.px)) {
        ...
        ShortFashionVehicle(
            shorts =  remember(feedProvider) { feedProvider.getAllFashionFeed().filter { it.typeId==2 }
            }
        )
        ...
        MainAllFashionGrid(
            fashions = remember(feedProvider) { feedProvider.getAllFashionFeed()
                .filter { it.typeId==1 }
            }
        )
    }
...
@Composable
private fun ShortFashionVehicle(modifier: Modifier = Modifier, shorts: List<Fashion>) {
