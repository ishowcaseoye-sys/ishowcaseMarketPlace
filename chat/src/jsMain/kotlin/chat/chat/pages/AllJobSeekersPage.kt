...
fun AllJobSeekersPage(query: String) {
    ...
    Column(modifier = Modifier.padding(top=90.px)) {
        ...
        ShortSuggestionsJobSeekers(
            shorts =  remember(feedProvider) { feedProvider.getAllJobSeekersFeed().filter { it.typeId==2 }
            }
        )

        Box(modifier = Modifier.height(30.px))

        MainAllJobSeekersGrid(
            jobSeekers = remember(feedProvider) {
...
@Composable
private fun ShortSuggestionsJobSeekers(modifier: Modifier = Modifier, shorts: List<JobSeeker>) {
