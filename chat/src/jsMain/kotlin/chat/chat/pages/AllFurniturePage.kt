...
@Composable
fun AllFurniturePage(query: String,) {
    ...
    Column(modifier = Modifier.padding(top=90.px)) {
        ...
        ShortFurnitureVehicle(
            shorts =  remember(feedProvider) { feedProvider.getAllFurnitureFeed().
            filter { it.typeId==2 }
            }
        )
        ...
        MainAllFurnitureGrid(
            furniture = remember(feedProvider) { feedProvider.getAllFurnitureFeed()
                .filter { it.typeId==1 }
            }
        )
    }
...
@Composable
private fun ShortFurnitureVehicle(modifier: Modifier = Modifier, shorts: List<Furniture>) {
