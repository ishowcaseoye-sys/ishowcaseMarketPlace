...
import chat.chat.components.widgets.player.PlayerAndCommentsFashion
...
@Composable
fun VideoFashionPlayerPage(
    id: String ,
    modifier: Modifier = Modifier
    ) {
    val navigator = LocalNavigator.current
    val fashions = convertFashionJsonStringToList ( fashionJsonString )
    val fashionList = fashions.filter ({it.id == id.toInt() } )
    var fashion : Fashion = fashionList.get(0)
    val videoId = BASE_URL + FILES + fashion.video1
    ...
    val segmentContent = remember {
        movableContentOf { modifier: Modifier ->
            SegmentedContent(category = "Fashions", state = selectedSegment, modifier = modifier)
        }
    }

    Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.TopEnd) {
        Row(modifier = Modifier.fillMaxSize().then(modifier)) {
            PlayerAndCommentsFashion(
