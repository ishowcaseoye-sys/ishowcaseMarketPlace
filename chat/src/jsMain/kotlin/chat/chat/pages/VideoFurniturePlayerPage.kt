...
import chat.chat.components.widgets.player.PlayerAndCommentsFurniture
...
@Composable
fun VideoFurniturePlayerPage(
    id: String ,
    modifier: Modifier = Modifier,
) {
    val pageId : String = remember { mutableStateOf(id).toString() }
    VideoFurniturePlayerPageImpl(id, modifier)
}

@Composable
fun VideoFurniturePlayerPageImpl(
    id: String ,
    modifier: Modifier = Modifier
    )
{
    val navigator = LocalNavigator.current
    val furnitures = convertFurnitureJsonStringToList ( furnitureJsonString )
    val furnitureList = furnitures.filter { it.id == id.toInt() }
    var furniture : Furniture = furnitureList.get(0)
    val videoId = BASE_URL + FILES + furniture.video1
    ...
    val segmentContent = remember {
        movableContentOf { modifier: Modifier ->
            SegmentedContent(category = "Furniture", state = selectedSegment, modifier = modifier)
        }
    }

    Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.TopEnd) {
        Row(modifier = Modifier.fillMaxSize().then(modifier)) {
            PlayerAndCommentsFurniture(
