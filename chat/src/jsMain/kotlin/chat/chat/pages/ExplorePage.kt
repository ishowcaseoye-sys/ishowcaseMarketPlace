...
import chat.chat.models.ExploreGridCategoryWithVideosPlus
import chat.chat.models.ExploreGridDetailsPlus
...
fun ExplorePage(modifier: Modifier = Modifier) {
    console.info("ExplorePage Begin 1")
    val feedProvider = remember { FeedProvider() }
    console.info("ExplorePage Begin 2")
    val fashionFeed =  feedProvider.getAllFashionFeed()
    val grids = remember(feedProvider) {
        //val trendingNowFeed = feedProvider.getTrendingNowFeed()
        //val musicFeed = feedProvider.getNewVideoFeed()

        console.info("ExplorePage Begin 3")
        listOf(
            ExploreGridDetailsPlus(
                asset = Asset.Icon.TRENDING_SELECTED,
                title = "Trending",
                categoriesWithVideos = listOf(
                    ExploreGridCategoryWithVideosPlus("Electronic", fashionFeed),
                    //ExploreGridCategoryWithVideos("Hotels", trendingNowFeed.shuffled()),
                    //ExploreGridCategoryWithVideos("JobSeekers", trendingNowFeed.shuffled()),
           ...
           /* ExploreGridDetails(
                ...
                title = "Music",
                categoriesWithVideos = listOf(
                    //ExploreGridCategoryWithVideos("Electronic", fashionFeed),
        ...
        console.info("ExplorePage Begin 4")
    }
    console.info("ExplorePage Begin 5")
    Column(
        modifier = Modifier.fillMaxWidth().then(modifier),
        verticalArrangement = Arrangement.spacedBy(CONTAINER_PADDING),
    ) {
        console.info("ExplorePage Begin 6")
      /*  grids.forEach {
            details -> ExploreGridSectionPlus(details)
        }*/


        /*fashionFeed.forEach { fashion ->
            console.info( "AXA - fashion title"+fashion.title)
            FashionThumbnailCard(
                fashion = fashion,
...
@Composable
private fun ExploreGridSectionPlus(details: ExploreGridDetailsPlus) {
    ...
    Column(
        ...
        Row(
            ...
            Image(
                modifier = Modifier.backgroundImage(Styles.Gradient.RED_TO_PINK)
                    .clip(Circle())
                    .padding(10.px),
                src = details.asset,
                width = 44,
        ...
        Crossfade(
            ...
            Box {
                Div(
                    ...
                    console.info("before populating Thumbnail")
                    videos?.forEach { details ->
                        FashionThumbnailCard(fashion = details, false, size = GRID_THUMBNAIL_CARD_SIZE)
                    }
                }
