...
import chat.chat.components.widgets.player.PlayerAndCommentsJobSeekers
...
@Composable
fun VideoJobSeekersPlayerPage(
    id: String ,
    modifier: Modifier = Modifier )
{
    val navigator = LocalNavigator.current
    val jobSeekers = convertJobSeekerJsonStringToList(jobSeekersJsonString)
    val jobSeekerList = jobSeekers.filter ( {it.id == id.toInt()} )
    var jobSeeker : JobSeeker = jobSeekerList.get(0)

    val videoId = BASE_URL + FILES + jobSeeker.video1
    ...
    val segmentContent = remember {
        movableContentOf { modifier: Modifier ->
            SegmentedContent(category = "JobSeekers", state = selectedSegment, modifier = modifier)
        }
    }

    Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.TopEnd) {
        Row(modifier = Modifier.fillMaxSize().then(modifier)) {
            PlayerAndCommentsJobSeekers(
