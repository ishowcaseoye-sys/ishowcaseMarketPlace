...
fun ShortVehicleThumbnailCard(
    ...
    console.info("ShortVehicleThumbnailCard URL = "+BASE_URL + FILES + details.video1)
    ...
    Column(
        modifier = Modifier
            ...
            .noShrink()
            .onClick {
                navigator.pushRoute(Route.Short(id = details.id.toString()))
                //ShortsGrid(true)
            }
        ...
        Column(modifier = Modifier.color(Styles.VIDEO_CARD_SECONDARY_TEXT)) {
            ...
            Box(Modifier.limitTextWithEllipsis()) {
                Text("${details.priceCurrency } views • ${details.description} ago")
            }
        }
    }
}

object ShortVehicleThumbnailCardDefaults {
    val SIZE = IntSize(width = 207, height = 368)
}
