...
import chat.chat.components.widgets.electronicAttr
...
@Composable
fun PlayerAndCommentsElectronics(
    ...
    Row() {
        ...
        Column(modifier = modifier.margin (top=1.px) ) {
            ...
            Box(modifier = modifier.minWidth(540.px)) {
                VideoPlayerProperty(
                    videoId = BASE_URL + FILES + electronic.video1,
            ...
            Row(
                ...
                SpacedRow(24) {
                    IconLabel(iconAsset = Asset.Icon.EYE, label = electronic.id.toString())
                    IconLabel(iconAsset = Asset.Icon.DATE, label = dateFormat(electronic.enteredDate))
                }
                SpacedRow(
                    spacePx = 10,
                    modifier = Modifier
        ...
        Column(modifier = modifier.margin (top=1.px).padding(left=16.px, right=16.px )) {
            ...
            Column(modifier = Modifier.flexWrap(FlexWrap.Wrap)) {
                ...
                TextBox(
                    ...
                    maxLines = 2,
                    size = 20,
                    text = "Model : "+ electronic.modelNumber,
                    weight = FontWeight.Normal,
                    modifier = Modifier.padding( top=4.px )
                )
            }
            ...
            Box(
                ...
                TextBox(
                    text =  electronic.priceCurrency.substring(0,1) +" "
                            + numberCalculation( electronic.priceMain )  ,
                    size = 30,
            ...
            Row() {
                ...
                Column() {
                    Box(modifier = Modifier.height(10.5.px))
                    TextBox(
                        text = electronic.marketedByName,
                        maxLines = if (isDescriptionBoxExpanded) null else 3,
                    )
                    TextBox(
                        text = electronic.marketedByPhone,
                        maxLines = if (isDescriptionBoxExpanded) null else 3,
                    )
                    TextBox(
                        text = electronic.marketedByEmail,
                        maxLines = if (isDescriptionBoxExpanded) null else 3,
                    )
                }
            }
