...
@Composable
fun ShortJobSeekersThumbnailCard(
    jobSeeker: JobSeeker,
    ...
    Column(
        modifier = Modifier
            ...
            .noShrink()
            .onClick {
                navigator.pushRoute(Route.VideoJobSeekers(id = jobSeeker.id.toString()))
                //ShortsGrid(true)
            }
        ...
        Column(modifier = Modifier.color(Styles.VIDEO_CARD_SECONDARY_TEXT)) {
            ...
            val amount = jobSeeker.yearsOfExperience.toString()+" yrs - "+
                        jobSeeker.expSalaryCurrency.substring(0,1) +" "+
                    numberCalculation( jobSeeker.expSalaryMain ).toString()
            ...
            Box(Modifier.limitTextWithEllipsis()) {
                TextBox( jobSeeker.description ,  size=14, maxLines=2 )
            }
        }
    }
}
