...
fun PropertyThumbnailCard(
    ...
    Column(
        modifier = Modifier
            .clickable {
                navigator.pushRoute(Route.VideoProperty(id = property.id.toString()))
            }
            .noShrink()
        ...
        Box(modifier = Modifier.fillMaxWidth()) {
            Image(
                ...
                src = BASE_URL + FILES + property.img1,
                width = size?.width,
                height = size?.height,
            )
            Box(
        ...
        Row(
            ...
            Column(
                ...
                Row () {
                    ...
                    Column() {
                        if (property.propUseId == 1) {
                            TextBox(
                                PROPERTY_USE[property.propUseId].toString(), color = Styles.RED
                            )
                        } else if (property.propUseId == 2)
                            TextBox(
                                PROPERTY_USE[property.propUseId].toString(),
                                color = Styles.RED_LIGHT
                            )
                        else if (property.propUseId == 3)
                            TextBox(
                                PROPERTY_USE[property.propUseId].toString(),
                                color = Styles.BLUE_BORDER
                            )
                        else if (property.propUseId== 4)
                            TextBox(
                                PROPERTY_USE[property.propUseId].toString(),
        ...
        if (!segment) {
            property.description.let { safeChannelName ->
                Row(
