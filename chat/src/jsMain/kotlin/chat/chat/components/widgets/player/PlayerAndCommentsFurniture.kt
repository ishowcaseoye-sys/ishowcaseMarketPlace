...
import chat.chat.components.widgets.furnitureAttr
...
@Composable
fun PlayerAndCommentsFurniture(
    ...
    val videoStr = BASE_URL + FILES + furniture.video1
    ...
    Row() {
        ...
        Column(modifier = modifier.margin (top=1.px) ) {
            ...
            Row(
                ...
                SpacedRow(24) {
                    IconLabel(iconAsset = Asset.Icon.EYE, label = furniture.id.toString())
                    IconLabel(iconAsset = Asset.Icon.DATE, label = dateFormat(furniture.enteredDate))
                }
                SpacedRow(
                    spacePx = 10,
                    modifier = Modifier
        ...
        Column(modifier = modifier.margin (top=1.px).padding(left=16.px, right=16.px )) {
            ...
            Column(modifier = Modifier.flexWrap(FlexWrap.Wrap)) {
                ...
                TextBox("Brand : "+furniture.brand,
                    modifier = Modifier.padding(top=8.px)
                        )

                TextBox("Color : "+furniture.color,
                    modifier = Modifier.padding(top=8.px))
            ...
            Box(
                ...
                TextBox(
                    text =  furniture.priceCurrency.substring(0,1) +" "
            ...
            Row() {
                ...
                Column() {
                    TextBox(
                        text = furniture.marketedByName,
                        maxLines = if (isDescriptionBoxExpanded) null else 3,
                    )
                    TextBox(
                        text = furniture.marketedByPhone,
                        maxLines = if (isDescriptionBoxExpanded) null else 3,
                    )
                    TextBox(
                        text = furniture.marketedByEmail,
                        maxLines = if (isDescriptionBoxExpanded) null else 3,
                    )
                }
            }
